	...
	<form id="add-form" method="post">
        <table>
            ...
            <tr>
                ...
                <td>
                	<select name="role" class="easyui-combobox" panelHeight="auto" style="width:268px" data-options="required:true, missingMessage:'请选择角色'">
		                <c:forEach items="${roleList }" var="role">
		                <option value="${role.id }">${role.name }</option>
		                </c:forEach>
		            </select>
            ...
            <tr>
                ...
                <td><input type="email" name="email"  class="wu-text easyui-emailbox easyui-validatebox" data-options="required:true,min:1,precision:0, missingMessage:'请填写email'" /></td>
            </tr>
            <tr>
	...
	<form id="edit-form" method="post">
        ...
        <table>
            ...
            <tr>
                <td width="60" align="right">所属角色:</td>
                <td>
                	<select id="edit-roleId" name="role" class="easyui-combobox" panelHeight="auto" style="width:268px" data-options="required:true, missingMessage:'请选择角色'">
		                <c:forEach items="${roleList }" var="role">
		                <option value="${role.id }">${role.name }</option>
		                </c:forEach>
		            </select>
            ...
            <tr>
                <td width="60" align="right">邮箱:</td>
                <td><input type="email" id="edit-email" name="email"  class="wu-text  easyui-validatebox" data-options="required:true,min:1,precision:0, missingMessage:'请填写email'" /></td>
            </tr>
            <tr>
                <td width="60" align="right">地址:</td>
...
<script type="text/javascript">
	...
	*/
	function openEdit(){
		...
		$('#edit-dialog').dialog({
            ...
            onBeforeOpen:function(){
            	...
            	$("#edit-roleId").combobox('setValue',item.role);
	...
	$("#search-btn").click(function(){
	    //先把角色id和性别的值取出，再根据用户名进行赋值
		var role = $("#search-role").combobox('getValue');
		var gender = $("#search-sex").combobox('getValue')
	...
	*/
	$('#data-datagrid').datagrid({
		...
		/*	{ field:'password',title:'密码',width:100},*/
			{ field:'role',title:'所属角色',width:100,
